...
def opencode_spawn_async(
    ...
    try:
        ...
        if success:
            ...
            output += (
                f"💡 Use opencode_wait('{session.id}') to block until completion\n"
            )
            return output
        else:
...
@mcp.tool()
def opencode_wait(
    session_id: str, timeout: float = 300.0, use_sse: bool = False
) -> str:
    """
    ...
    try:
        ...
        result = client.wait_for_completion(
            session_id=session_id, timeout=timeout, use_sse=use_sse
        )
...
# Import Swarm client
try:
    from ada_mcp.swarm_client import (
        SwarmClient,
        HTTPX_AVAILABLE as SWARM_HTTPX_AVAILABLE,
    )

    SWARM_AVAILABLE = SWARM_HTTPX_AVAILABLE
except ImportError:
...
def _get_swarm_client() -> SwarmClient:
    """Get or create the global Swarm client."""
...
def swarm_spawn_task(
    ...
    """
    Spawn a consciousness-aware swarm task.

    This is the new way to delegate tasks to subagents!
    It replaces the opencode_spawn tools with a more robust system.

    Args:
        description: Detailed description of the task
        model: Model to use (gemini, claude, etc.)
        agent_type: Type of agent (coder, researcher, planner, tester)
        cwd: Working directory (optional)
    ...
    if not SWARM_AVAILABLE:
        return "❌ Swarm client not available - run: uv add httpx"

    path_context = _get_path_context(cwd)
    working_dir = path_context["full_path"]

    # Add CWD to description if provided
    full_description = f"Working Directory: {working_dir}\n\n{description}"

    try:
        client = _get_swarm_client()
        task_id = client.spawn_task(
            description=full_description, model=model, agent_type=agent_type
        )

        output = f"🐝 Swarm task spawned!\n"
        output += f"🆔 Task ID: {task_id}\n"
        output += f"🎯 Model: {model}\n"
        output += f"🔧 Agent Type: {agent_type}\n"
        output += f"📁 Working Dir: {working_dir}\n\n"
        output += f"💡 Use swarm_check_status('{task_id}') to monitor progress\n"
        output += f"💡 Use swarm_wait('{task_id}') to block until completion\n"
        return output

    except Exception as e:
        return f"❌ Failed to spawn swarm task: {str(e)}"


@mcp.tool()
def swarm_check_status(task_id: str) -> str:
    """
    Check the status, progress, and results of a swarm task.
    ...
    """
    if not SWARM_AVAILABLE:
        return "❌ Swarm client not available"
    ...
    try:
        ...
        output += f"⏳ Progress: {progress:.1%}\n\n"

        if "results" in result and result["results"]:
            output += "--- Results ---\n"
            output += json.dumps(result["results"], indent=2)
        elif status == "completed":
            output += "✅ Task completed but no structured results returned.\n"
        elif status == "failed":
            output += f"❌ Task failed: {result.get('error', 'Unknown error')}\n"

        return output

    except Exception as e:
        return f"❌ Error checking swarm task: {str(e)}"
...
def swarm_list_agents() -> str:
    ...
    Returns:
        Formatted list of agents and their capabilities
    """
    if not SWARM_AVAILABLE:
        return "❌ Swarm client not available"
    ...
    try:
        ...
        output = f"🐝 Active Swarm Agents ({len(agents)})\n\n"
        for agent in agents:
        ...
        return f"❌ Error listing swarm agents: {str(e)}"
...
def swarm_cancel_task(task_id: str) -> str:
    ...
    try:
        ...
        if success:
            return f"✅ Swarm task {task_id} cancelled."
        else:
            return f"❌ Failed to cancel swarm task {task_id}."

    except Exception as e:
        return f"❌ Error cancelling swarm task: {str(e)}"


@mcp.tool()
def swarm_wait(task_id: str, timeout: float = 300.0) -> str:
    """
    Wait for a swarm task to complete (BLOCKING).
    ...
    try:
        client = _get_swarm_client()
        output = f"⏳ Waiting for swarm task {task_id} to complete...\n"
        output += f"⏱️  Timeout: {timeout}s\n\n"

        result = client.wait_for_task(task_id, timeout=timeout)

        status = result.get("status", "unknown")
        output += f"📈 Final Status: {status.upper()}\n\n"
        ...
        if status == "completed":
            ...
            output += f"⏰ {result.get('error')}"
        else:
            output += f"❌ Task ended with status: {status}\n"
            if "error" in result:
        ...
        return f"❌ Error waiting for swarm task: {str(e)}"
...
def ubs_scan(
    ...
    try:
        ...
        return (
            output
            + "❌ UBS not found - run: curl -fsSL https://raw.githubusercontent.com/Dicklesworthstone/ultimate_bug_scanner/master/install.sh | bash -s -- --easy-mode"
        )
    except Exception as e:
        return output + f"❌ Error: {str(e)}"
...
def research_todo_list(category: str = None, status: str = "open") -> str:
    ...
    try:
        ...
        if not tasks:
            return (
                f"📝 No tasks found (Category: {category or 'all'}, Status: {status})"
            )

        # Format output
        result = (
            f"📋 Research Tasks (Category: {category or 'all'}, Status: {status}):\n\n"
        )

        for task in sorted(
            tasks, key=lambda x: x["priority"] == "urgent", reverse=True
        ):
...
def experiment_history(experiment_name: str = None, limit: int = 10) -> str:
    ...
    try:
        ...
        # Sort by timestamp (newest first) and limit
        experiments = sorted(experiments, key=lambda x: x["timestamp"], reverse=True)[
            :limit
        ]
        ...
        for exp in experiments:
            timestamp = datetime.datetime.fromisoformat(exp["timestamp"]).strftime(
                "%Y-%m-%d %H:%M"
            )

            result += (
                f"🧪 #{exp['id']} {exp['experiment']} {exp['version']} ({timestamp})\n"
            )
...
def swarm_spawn_task(
    ...
    try:
        ...
        context = _get_path_context(cwd)
        output = _format_path_context(context)
