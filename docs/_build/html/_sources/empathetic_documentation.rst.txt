...
1. **What NOT to do** (the anti-pattern)
2. **Why it seems right** (acknowledging the reasoning)
3. **Why it's wrong here** (context-specific explanation)
4. **What to do instead** (correct approach with examples)
...
1. **``.ai/GOTCHAS.md``** — Primary anti-pattern documentation
...
2. **``@ai-warning`` annotations** — Inline warnings in source code

   Example from ``docs/Makefile``::

       # @ai-warning: Don't run 'make html' manually in production - docs build 
       # automatically during Docker image build (see frontend/Dockerfile lines 14-15).
       # Only run locally for testing outside containers.

3. **Detection patterns** — Teaching AI to recognize its own suggestions
...
- AI assistants correctly avoided suggesting ``make html`` after reading GOTCHAS.md
- Self-correction happened faster with empathetic framing vs. simple negation
...
1. **Quantitative measurement:** Can we measure the efficiency improvement in AI context usage?
2. **Transfer learning:** Does this pattern help AI assistants generalize to similar codebases?
3. **Human pedagogy:** Would this framing improve technical documentation for human learners?
4. **Cross-domain application:** Does empathetic framing work for non-code documentation?
...
1. Add them to ``.ai/GOTCHAS.md`` following the empathetic framing structure
2. Consider adding ``@ai-warning`` annotations to relevant source files
3. Update detection patterns if you notice recurring AI suggestions
4. Document your observations — you're contributing to an emerging research area

See Also
========

- :doc:`development` — General development practices
- :doc:`specialists` — Specialist system architecture
- ``.ai/GOTCHAS.md`` — Complete anti-pattern reference
- ``.ai/CONVENTIONS.md`` — Documentation placement strategy
