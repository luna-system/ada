...
class OllamaEmbeddingFunction:
    ...
    Expected interface: __call__(input: List[str]) -> List[List[float]]
    """
    ...
    def name(self) -> str:
        """Return function name for ChromaDB compatibility."""
        ...
        if self.use_http:
            ...
            for attempt in range(5):
                try:
                    ...
                    if attempt < 4:
                        logger.warning(f"ChromaDB not ready (attempt {attempt+1}/5), retrying in 2s...")
                        time.sleep(2)
                    else:
                        logger.error(f"Failed to connect to ChromaDB after 5 attempts: {e}")
                        raise
        else:
